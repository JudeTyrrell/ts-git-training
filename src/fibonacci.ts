export const computeFibonacciNumber = (position: number, recursion: boolean = false, negativeRecursion: boolean = false): number => {
    if (recursion) {
        return recursiveFibonacci(1, 1, position - 2);
    }
  
    if (negativeRecursion) return negativeRecursiveFibonacci(position);
